...
Java 内存模型（JMM）在 JVM 层的实现涉及**字节码生成**、**内存屏障插入**、**对象头管理** 和 **垃圾回收** 等多个层面。
...
例如，线程 A 的`CAS(value, 1, 2)`操作必然 happens-before 线程 B 的 `CAS(value, 2, 3)` 操作，因为 B 的 CAS 依赖 A 的结果，硬件会保证这种顺序。
